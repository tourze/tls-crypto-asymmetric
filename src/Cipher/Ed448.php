...
class Ed448 implements AsymmetricCipherInterface
{
    ...
    private function isEd448Supported(): bool
    {
        ...
        return in_array('ED448', $curves, true);
    }
    ...
    private function mockSign(string $data, string $privateKey): string
    {
        ...
        $decodedKey = base64_decode($keyContent, true);
        if (false === $decodedKey) {
            throw new AsymmetricCipherException('Ed448私钥格式无效，无法解码');
        }
    ...
    private function mockVerify(string $data, string $signature, string $publicKey): bool
    {
        ...
        $decodedPublicKey = base64_decode($publicKeyContent, true);
        if (false === $decodedPublicKey) {
            throw new AsymmetricCipherException('Ed448公钥格式无效，无法解码');
        }
    ...
    public function sign(string $data, string $privateKey, array $options = []): string
    {
        ...
        try {
            ...
            if (false === $details || OPENSSL_KEYTYPE_EC !== $details['type']) {
